...
if st.session_state.idea and st.session_state.image_url and st.button("🚀 Automate Merch Upload (Zapier)"):
    if zapier_webhook:
        payload = {
        ...
        if response.status_code == 200:
            st.success("📤 Sent merch data to Zapier!")
        else:
            st.error(f"❌ Zapier returned an error: {response.status_code}")
    else:
        st.warning("⚠️ Please enter your Zapier webhook in the sidebar.")
